...
if (document.querySelector(".autocomplete")) {
    ...
    let brandsArray = document.querySelectorAll(".check-box--brands .check-box__text");
    ...
    var brandsAutoComplete = {
        ...
        listenAction: () => {
            document.querySelector(".js-brands-input").addEventListener("input", function(event) {
                brandsAutoComplete._showAllBrands(this);
                let newArr = brandsAutoComplete._qwerty(this.value.toLowerCase().trim(), brandsArray);
                ...
                if (this.value.length > 0 && newArr.length === 0) {
                    noResult.classList.remove("visually-hidden");
                }
            });
        },

        _showAllBrands: (element) => {
            let brandsList = element.closest(".filters__content").querySelector(".check-box");
            brandsList.classList.remove("hide");
            btn.classList.add("visually-hidden");
        },

        _qwerty: (element, arrayElement) => {
